...
retry:
	if((rtv = sem_wait(mc_startup->s_lock.l_key)) == -1 && errno == EINTR)
		goto retry;
	if(rtv == 0)
		mc_startup->s_lock.l_pid = getpid();
	return rtv;
}
...
mc_unlock(void)
{
	if(sem_post(mc_startup->s_lock.l_key))
		return -1;	
...
mc_init_event(int maxfds)
{
	...
	if((p = calloc(1, MC_EVENT_SIZE)) == NULL)
		return NULL;
	p->maxfds = maxfds;
#ifdef MC_USE_KQUEUE
	if((p->fd = kqueue()) == -1){
